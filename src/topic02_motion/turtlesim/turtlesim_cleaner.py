...
def move(velocity_publisher, speed, distance, is_forward):
    # declare a tiwst message to send velocity commands
    velocity_message = Twist()

    # get current location
    global x,y

    # store starting point x,y
    x0 = x  
    y0 = y
    ...
    # before terminating the program, make sure to publish stop command
...
if __name__ == '__main__':
    try:
        ...
        # declare pose subsriber
        position_topic = '/turtle1/pose'
        pose_subscriber = rospy.Subscriber(position_topic, Pose, poseCallback)

        # add a short delay, so ctrl+c can actually kill the loop
        time.sleep(2)
        ...
        #rotate(velocity_publisher, 30.0, 90.0, True)
